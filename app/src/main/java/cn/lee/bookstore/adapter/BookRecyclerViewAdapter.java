 ...
 */
public class BookRecyclerViewAdapter extends RecyclerView.Adapter implements ItemTouchHelperAdapter {
    ...
    public BookRecyclerViewAdapter(List<BookDetail.ResultEntity.DataEntity> mData, Activity mActivity) {
        this.mData = mData;
        this.mActivity = mActivity;
        DBManager.addOnDBChange(new DBManager.OnDBChange() {
            @Override
            public void onDBChange() {
                BookRecyclerViewAdapter.this.notifyDataSetChanged();
            }
        });
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View item=null;
        if(BookPageFragment.isList){
            item= LayoutInflater.from(mActivity).inflate(R.layout.item_book_list,parent,false);
        }else{
            item= LayoutInflater.from(mActivity).inflate(R.layout.item_book_grid,parent,false);
        }
...
//        myholder.tv_book_title.setText("《"+data.title+"》");
    ...
    private void showBookStore(final int position) {//弹出对话框让用户选择各大商城
        if(!TextUtils.isEmpty(mData.get(position).online)){//如果存在Url才显示
            ...
            for (int i=0;i<stores.length;i++){
                ...
                cardView.setBackgroundResource(R.color.list_background);//设置背景
    ...
    public void onItemMove(int fromPosition, int toPosition) {
        ...
        /*BookDetail.ResultEntity.DataEntity dataFrom = mData.get(fromPosition);
        BookDetail.ResultEntity.DataEntity dataTo = mData.get(toPosition);
        BookDetail.ResultEntity.DataEntity temp=dataFrom;
        mData.set(fromPosition,dataTo);
        mData.set(toPosition,temp);*/
